...
import GuestCheckInQr from './GuestCheckInQr.vue'
...
const { prefilledName, checkInGuest } = usePublicGuestContext(() => props.invitation)
...
const { rsvpSubmitting, rsvpSuccess, rsvpError, lastRsvpGuest, submitRsvp } = usePublicGuestActions(
  () => props.slug,
)

const qrGuest = computed(() => {
  const fromSubmit = lastRsvpGuest.value
  if (fromSubmit?.rsvp === 'hadir' && fromSubmit.checkInToken) return fromSubmit
  return checkInGuest.value
})
...
<template>
  <div class="space-y-4">
    ...
    <GuestCheckInQr
      v-if="qrGuest?.checkInToken"
      :token="qrGuest.checkInToken"
      :guest-name="qrGuest.name"
    />
  </div>
</template>
